...
## 7. Decision — 2026-09-02

**Steady Harbor (blue / grey / black) is the theme.** PO decision; the amber/harbor A/B is retired before it collected a statistically useful sample. Implementation: harbor's light and dark tokens are now the Daybreak base in `daybreak.css`, the `data-palette` override blocks are removed, `getPaletteVariant()` is pinned to `'harbor'` (kept so analytics properties and the PDF theming call sites stay stable), and the report PDF defaults to harbor. Semantic colors (signal/review/none/urgent) were never part of the experiment and are unchanged.
