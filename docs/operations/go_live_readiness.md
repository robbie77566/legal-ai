...
| Domain/TLS/email DNS | **DNS CUTOVER DONE (verified live 2026-09-02):** apex → Render A 216.24.57.1, cert issued (Google Trust), 301 → https://www; www + api CNAMEs live with certs, healthz 200; GoDaddy Website Builder placeholder and forwarding removed. **Resend domain VERIFIED (2026-09-02, evening):** DKIM + send-subdomain SPF (→ amazonses via Domain Connect) + feedback MX were live by morning, but the domain sat in *Pending* in Resend until "Verify DNS Records" was clicked — every send in between was refused with 'domain is not verified' (visible only in the api log, never in Resend's dashboard). Ops console now has a one-click transport test (Email tile → Send test email to me). Caution learned: Resend's auto-configure WIPED the root SPF (breaking admin@'s M365 outbound auth — no M365 DKIM selectors exist either); re-added `v=spf1 include:secureserver.net -all` at root, verified propagated. **admin@ mailbox (2026-09-02):** there never WAS an M365 mailbox (the outlook MX was a GoDaddy artifact, wiped in the churn) — inbound now via GoDaddy free email forwarding admin@ → founder Gmail (GoDaddy manages the MX), outbound via Gmail send-as through Resend SMTP (smtp.resend.com, dedicated API key, DKIM-aligned). App-admin password reset completed through the live forgot-password flow + Resend |
