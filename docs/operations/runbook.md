...
| Payment succeeded but no case created | `checkout.session.completed` webhook not landing — wrong/stale `STRIPE_WEBHOOK_SECRET` (dev: it changes per `stripe listen` session; prod: endpoint secret from the Stripe dashboard). **Seen 2026-09-12:** the Stripe account had **no webhook endpoint at all** (the secret in Render was orphaned), and the hourly sweep's timer was reset by every deploy, so it never ran on a release day | Check webhook delivery 200s in Stripe first (`stripe.webhookEndpoints.list` must show `https://api.snotnoselegal.com/webhooks/stripe`, enabled, in the SAME mode as the secret key — the endpoint `we_1UEwCJDCwjlvhlyf74otHX5g` was created via the API in test mode on 2026-09-12 and its secret installed in Render). Three nets now: (1) the webhook; (2) the success page's own poll asks Stripe and fulfills a paid session for that buyer itself (`GET /checkout/fulfillment` self-heal, logged as a warning — a healed count above zero still means the webhook is broken); (3) reconciliation runs 30 s after every start and then hourly. Test cards (`4242…`, decline/3DS variants — matrix in README §How to test) work only in test mode; prod is verified by the launch-day real-money smoke (go_live_readiness §6). **When the live keys go in, create the live-mode endpoint and install ITS secret — the test-mode endpoint does not carry over.** |
