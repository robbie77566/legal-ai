...
- **OPS-2 Refunds** *(built 2026-09-07 — `/ops/money`, `Refund` ledger, partial refunds, cohort refund rate; PM-report export and fee sync still open)*: issue full/partial refund per E-1/E-6 policy with reason enum; Stripe-linked; append-only audit entry; weekly refund total surfaces on the console and in the PM report (5% reserve tracking).
...
- **OPS-4 Retention & deletion:** dashboard of cases approaching 12-month deletion; verified-request deletion flow (identity check → soft-delete grace → hard delete incl. S3 objects **and their versions**, vectors, entity rows, report artifacts — propagating through backups within ≤35 days) with audit trail; consent-revocation (US-5) processed here. **Deletion is scoped per the retention matrix** (`../architecture/mvp_v1_system_design.md` §11a.2): the payment ledger, the disclosure-ack archive, and the pseudonymized PII-minimal audit/event skeleton survive by design; the deletion certificate is itself an event. The console shows the operator exactly what is deleted vs. retained, so support answers accurately.
- **OPS-5 User & reviewer management** *(partly built 2026-09-08 — `SUPPORT` role, invite email + set-password flow, resend; deactivate/MFA/last-admin guard still open, see staff_console.md §9)*: existing RBAC surfaces (invite QA reviewers as ATTORNEY role, deactivate, reset MFA) — reuse the auth spec's admin flows, don't rebuild.
- **OPS-6 Support context** *(built 2026-09-08 — contact log on the case file, `support.contacted` event, request-to-admin flow; canned responses still open)*: every support contact (`support.contacted`) links to the case timeline; canned responses for the top scenarios (records how-to, delay notice, refund status) maintained here with R-5-reviewed copy.
...
**Requirements:** **PMX-1** the weekly report of `analytics_experimentation_plan.md` §5 is automated (no hand-built decks); **PMX-2** experiment flags have an admin UI (create, allocate, freeze) with every change audit-logged; **PMX-3** dashboards (funnel, outcome mix, guardrails, unit economics) accessible without SQL, **served from the nightly PII-stripped reporting schema — never the production OLTP** (system design §11a.6), with finance numbers from server-side mirrors + the payment ledger; **PMX-4** the confusion log (QA reviewers') lives in the QA console with one-click export into the weekly report; **PMX-5** doc-diff discipline — readouts link the spec commit (analytics plan §6) — is tracked as a checklist item in the readout template itself.
